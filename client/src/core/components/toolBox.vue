      ...
      <v-card>
        ...
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs3 v-for="(action, index) in actions" :key="index">
                <v-tooltip bottom open-delay="500">
                  <template v-slot:activator="{ on }">
                    <v-btn text v-on="on" @click="handleEvent(action.name)"> <v-icon v-if="action.icon"> {{ action.icon }} </v-icon> <span v-if="!action.icon"> {{ translate(action.name) }} </span></v-btn>
                  </template>
