module.exports = {
  createFirstStatement (filter) {
    let statement = 'select * from Users where 1 = 1 #filter# order by email asc Limit 1'
  ...
  createLastStatement (filter) {
    let statement = 'select * from Users where 1 = 1 #filter# order by email desc Limit 1'
    const filterstatement = this.createFilterstatement(filter)
    statement.replace(/#filter#/,filterstatement)
    return statement
  },
